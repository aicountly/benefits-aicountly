...
| --- | --- | --- | --- |
| Deploy to cPanel Production | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://benefits.aicountly.com (+ `/api`) |
| Deploy to cPanel Sandbox | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://benefits.gh.aicountly.com (+ `/api`) |
...
`https://benefits.aicountly.com/api/.env` unless Apache is told otherwise.
