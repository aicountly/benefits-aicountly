# AICOUNTLY auth workflow (Benefits)

How Benefits signs a user in. This is the shared AICOUNTLY SaaS flow — the same
...
1. User opens `benefits.aicountly.com` (or `benefits.gh.aicountly.com`).
2. No `auth_token` → redirect to
   `{portal}/login/authentication_jump/benefits?returnUrl={origin}/auth/callback`.
...
| Benefits host | Login redirect | Auth API | Product API |
|---|---|---|---|
| `benefits.aicountly.com` | `my.aicountly.com` | `my.aicountly.com` | `benefits.aicountly.com/api` |
| `benefits.gh.aicountly.com` | `sandbox.aicountly.com` | `my.aicountly.com` | `benefits.gh.aicountly.com/api` |
...
`benefits` out of either hostname, and `isSandboxHost()` picks the portal.
...
curl https://benefits.gh.aicountly.com/api/health
...
curl -i -X POST https://benefits.gh.aicountly.com/api/global/seskey

# 3. Unrelayed paths are refused
curl -i -X POST https://benefits.gh.aicountly.com/api/global/login   # expect 404
```
